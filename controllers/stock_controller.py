...
class StockController:
    ...
    def update_stock_prices_and_volume(self):
        prev_stock_value, current_stock_value, volume = (self.market_service.
                                                         get_stock_prices_and_volume(self.stock_symbol))
        value_diff = current_stock_value - prev_stock_value
        if prev_stock_value != 0:
            value_diff_percent = (value_diff / prev_stock_value) * 100
        else:
            value_diff_percent = 0
        self.view.set_stock_value(current_stock_value, value_diff, value_diff_percent)
        self.view.set_stock_volume(volume)

    def handle_buy_stock(self):
        self.buy_controller.set_stock_symbol(self.stock_symbol)
        self.buy_controller.set_user_id(self.main_controller.current_user.id)
